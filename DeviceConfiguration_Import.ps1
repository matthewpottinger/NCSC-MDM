...
$AADGroupConfigure = 'Do you want to configure Group Assignment [Y/N]'
     $response = Read-Host -Prompt $AADGroupConfigure
    if ($response -eq 'y') {
        ...
        write-host "Device Configuration policie will be created with no permissions" -ForegroundColor red
    }
...
If ($DuplicateDCP -eq $null) 
  {
    ...
    if ($response -eq 'y') 
    {
        ...
        Write-Host "Device ConfigID '$DeviceConfigID' and Target '$TargetGroupID'"
        Add-DeviceConfigurationPolicyAssignment -ConfigurationPolicyId $DeviceConfigID -TargetGroupId $TargetGroupId
    } 
  }  
 else 
